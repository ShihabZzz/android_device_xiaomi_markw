...
static int get_fade_index(int base, int num, int shift_fade) {
  if (num == 0)
    return 0; // we can ignore zeroes
  if ((base % num) != 0)
    return -1; // when values is not divisible
...
static void adapt_colors_for_blink(int *red, int *green, int *blue) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    part[i] = map_to_grid(
        part[i],
        interval); // round off values to the grid in specified interval
    if ((part[i] > 0) && (part[i] < base))
      base = part[i];
  }
...
Light::Light(std::pair<std::ofstream, uint32_t> &&lcd_backlight,
    ...
    : mLcdBacklight(std::move(lcd_backlight)), mOldLedDriver(old_led_driver),
      mRedLed(std::move(red_led)), mGreenLed(std::move(green_led)),
      mBlueLed(std::move(blue_led)), mRedBlink(std::move(red_blink)),
      mGreenBlink(std::move(green_blink)), mBlueBlink(std::move(blue_blink)),
      mRedBreath(std::move(red_breath)), mGreenBreath(std::move(green_breath)),
      mBlueBreath(std::move(blue_breath)) {
  auto attnFn(
      std::bind(&Light::setAttentionLight, this, std::placeholders::_1));
  auto backlightFn(
      std::bind(&Light::setLcdBacklight, this, std::placeholders::_1));
  auto batteryFn(
      std::bind(&Light::setBatteryLight, this, std::placeholders::_1));
  auto buttonsFn(
      std::bind(&Light::setButtonsBacklight, this, std::placeholders::_1));
  auto notifFn(
      std::bind(&Light::setNotificationLight, this, std::placeholders::_1));
...
void Light::setLcdBacklight(const LightState &state) {
  ...
  if (mLcdBacklight.second != DEFAULT_MAX_BRIGHTNESS) {
    ...
    LOG(VERBOSE) << "scaling brightness " << old_brightness << " => "
                 << brightness;
  }
...
void Light::setSpeakerLightLocked(const LightState &state) {
  ...
  if (mOldLedDriver) {
    ...
    if (blink) {
      ...
      // Indexes [0: 0.13, 1:0.26, 2: 0.52, 3:1.04, 4: 2.08, 5: 4.16, 6: 8.32,
      // 7: 16.64] max fade grid     256     128      64      32       16 8 4 to
      // do: match max fade element to grid
      max = (red > green) ? ((red > blue) ? red : blue)
      ...
      LOG(VERBOSE) << "Light::setSpeakerLightLocked:"
                   << " rf " << red_fade << ", gf " << green_fade << ", bf "
                   << blue_fade;

      if (red_fade == -1 || green_fade == -1 ||
          blue_fade == -1) { // to do: remove
    ...
    mRedBreath << red_fade << " " << (int)(onMs / 1000) << " " << red_fade
               << " " << (int)(offMs / 1000) << std::endl;
    mGreenBreath << green_fade << " " << (int)(onMs / 1000) << " " << green_fade
                 << " " << (int)(offMs / 1000) << std::endl;
    mBlueBreath << blue_fade << " " << (int)(onMs / 1000) << " " << blue_fade
                << " " << (int)(offMs / 1000) << std::endl;
