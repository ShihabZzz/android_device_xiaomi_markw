...
static inline int amplifier_enable() { return write_int(DEVICE_PATH, 1); }

static inline int amplifier_disable() { return write_int(DEVICE_PATH, 0); }

static int amp_set_input_devices(amplifier_device_t *device, uint32_t devices) {
  return 0;
}

static int amp_set_output_devices(amplifier_device_t *device,
                                  uint32_t devices) {
  return 0;
}
